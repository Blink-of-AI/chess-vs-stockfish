...
export default function Home() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '16px', gap: '16px' }}>
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', width: '100%', maxWidth: '900px' }}>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'stretch', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {state.phase === 'thinking' && (
                  <span style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                    {[0, 1, 2].map(i => (
                      <span key={i} className="thinking-dot" style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--accent)', display: 'inline-block' }} />
                    ))}
                  </span>
                )}
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: 'clamp(280px, 56vmin, 560px)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                  You ({state.playerColor === 'w' ? 'White' : 'Black'})
                </span>
              </div>
              <CapturedPieces pieces={capturedByPlayer} color={playerColorSafe} />
            </div>
            <Controls phase={state.phase} drawOfferState={state.drawOfferState} onResign={game.resign} onOfferDraw={handleDrawOffer} onNewGame={game.newGame} />
          </div>
        </div>
      ...
      {state.phase === 'promotion' && state.promotionPending && (
        <PromotionModal color={playerColorSafe} onSelect={game.completePromotion} />
      )}
    </div>
  );
}
